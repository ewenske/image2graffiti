...
namespace JSRCustomGraffitiManager
{
    public partial class Form1 : Form
    {
        ...
        private void buttonOpenJSRSave_Click(object sender, EventArgs e)
        {
            ...
            {
                jsrSavePath = folderBrowserDialog1.SelectedPath;
                PopulateList();
                SaveConfig();
            }
        }
        ...
        private void PopulateList()
        {
            ...
            try
            {
                ...
                foreach (string s in filePaths)
                {
                    ...
                    if (fileName.StartsWith("jetset___lar") || fileName.StartsWith("jetset___sma") || fileName.StartsWith("jetset___xla"))
                    {
                        files.Add(s);
                        listBoxSprays.Items.Add(fileName);
                    }
                }
            }
            catch
        ...
        private void DisplayGraffiti(string path)
        {
            ...
            else if (graffitiSize == GraffitiSize.ExtraLarge)
            {
                ...
                pictureBoxXL.Visible = true;
            }

            fs.Close();
        }

        private void listBoxSprays_SelectedIndexChanged(object sender, EventArgs e)
        {
            DisplayGraffiti(files[listBoxSprays.SelectedIndex]);
        }

        private void buttonDeleteCurrentGraffiti_Click(object sender, EventArgs e)
        {
            if (listBoxSprays.SelectedItem != null)
            {
                if (MessageBox.Show("Really delete " + listBoxSprays.SelectedItem + "?", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    File.Delete(files[listBoxSprays.SelectedIndex]);
                    PopulateList();
